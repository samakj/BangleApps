...
exports.gbHandler = (event) => {
  var HANDLERS = {
    ...
    notify: function () {
      ...
      if (
        lastMsg &&
        event.body == lastMsg.body &&
        lastMsg.src == undefined &&
        event.src == "Messages"
      ) {
    ...
    musicinfo: function () {
      require("messages").pushMessage(
        Object.assign({}, event, { t: "modify", id: "music", title: "Music" })
      );
    },
    ...
    alarm: function () {
      ...
      for (var j = 0; j < event.d.length; j++) {
        ...
        var last =
          event.d[j].h * 3600000 + event.d[j].m * 60000 < currentTime
            ? new Date().getDate()
            : 0;
    ...
    "calendar-": function () {
      ...
      else cal = cal.filter((e) => e.id != event.id);
      require("Storage").writeJSON("android.calendar.json", cal);
    },
    ...
    http: function () {
      ...
      else request.r(event); //r = resolve function
    },
    ...
    gps: function () {
      ...
      if (event.long !== undefined) {
        // for earlier Gadgetbridge implementations
    ...
    act: function () {
      ...
      if (exports.actHRMHandler) exports.actHRMHandler = undefined;
    ...
    listRecs: function () {
      let recs = require("Storage")
        .list(/^recorder\.log.*\.csv$/, { sf: true })
        .map((s) => s.slice(12, 21))
        .filter((s) => s.length > 7 /*ignore 'old' tracks without date*/);
      if (event.id && event.id.length > 2) {
        // Handle if there was no id supplied. Then we send a list all available recorder logs back.
    ...
    fetchRec: function () {
      ...
      if (event.id == "stop") {
        ...
        let log = require("Storage").open(
          "recorder.log" + event.id + ".csv",
          "r"
        );
        ...
        exports.gbSend({
          ...
          lines: "erase",
          cnt: pkgcnt,
        }); // "erase" will prompt Gadgetbridge to erase the contents of a already fetched log so we can rewrite it without keeping lines from the previous (probably failed) fetch.
        let sendlines = () => {
          lines = log.readLine();
          for (var i = 0; i < 3; i++) {
    ...
    accelsender: function () {
      require("Storage").writeJSON("accelsender.json", {
...
exports.httpHandler = (url, options) => {
  ...
  if (Bangle.httpRequest === undefined) Bangle.httpRequest = {};
...
exports.overwriteGPS = () => {
  // if the overwrite option is set, call this on init..
  ...
  Bangle.setGPSPower = ((isOn, appID) => {
    ...
    if (!this.lastGPSEvent) {
      ...
      if (!isOn && Bangle._PWR.GPS.includes(appID))
        Bangle._PWR.GPS.splice(Bangle._PWR.GPS.indexOf(appID), 1);
  ...
  setTimeout(() => {
    if (!Bangle.isGPSOn())
      require("android").gbSend({ t: "gps_power", status: false });
  }, 3000);
};
